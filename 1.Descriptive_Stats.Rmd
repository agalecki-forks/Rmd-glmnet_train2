...
```
Raw data are stored in `r xlsx_path` file contain `r nrow(scrambled_orig)` rows.  
...
scrambled2 <- scrambled  %>% 
          mutate(Gender=case_when(SEX==1 ~"Male",SEX==2 ~"Female"),
              Status=case_when(CASE_CONTROL==1 ~"Case",CASE_CONTROL==0 ~"Non-case"))%>%                                
...
```{r, histogram, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
```
## Cluster dendrogram for proteins

```{r Correlation-proteins, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Correlation-proteins-ord, echo=FALSE, message=FALSE, warning=FALSE,fig.width = 60, fig.height=40}

ord <- hcl$order
cors_ord <- cors[ord,ord]
cors_ord <- round(cors_ord,2)
corrplot.mixed(cors_ord, lower ="pie", upper="number", tl.pos ="lt",
 mar = c(0,0,2,0), tl.cex=4, title="Correlation matrix", number.cex =3) # left/top
#corrplot(cors_ord, method ="number",type ="upper", tl.pos ="n", add=TRUE,  number.cex=3, 
#   col="black", cl.pos ="n", main="Correlation matrix")
```
