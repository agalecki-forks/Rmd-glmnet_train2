...
Plot path of (standardized) coefficients (y-axis) versus $\ell_1$-norm.
...
We will use `ggplot()` function to create cross-validation curve and upper and lower standard deviation curves along
...
rbind(a0_min,a0_1se)  %>% unnest(beta)
...
betas %>%  print(n = 26)
 ...
 lj1
 left_join(lj1, cva_beta, by = c("a_idx", "step")) %>% 
   select(-dev.ratio)  %>% print( n=200)
