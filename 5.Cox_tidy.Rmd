...
* Using tibble created by `myglance`
...
test_rows <- 1:100 # nrow(data_drop_na) # Select rows for testing. Possibly all rows in data_drop_na.
#x_test <- x[test_rows,]
#y_test <- y[test_rows,]
...
dtx_test <- subset(data_test, select=-c(time,status))
x_test <- model.matrix(~0 +., data=dtx_test)
dim(x_test)
colnames(x_test)
# Create Surv object
y_test <- data.matrix(data_test[,c("time", "status")])
ySurv_test <- survival::Surv(y_test[,"time"], y_test[, "status"])
...
mod_all <- map(alphv, function(a){
  ...
  set_engine("glmnet") %>% 
  fit(Surv(time, status) ~ ., data = data_train)
})
length(mod_all)
tmp <- mod_all[[2]]
class(tmp)
