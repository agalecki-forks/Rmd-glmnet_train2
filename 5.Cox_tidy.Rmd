...
* Proteins: `prot_npx`
* Time/status vars: `surv_vars`
* Clinical variables: `clin_vars`
...
* Data preparation: Vector `vars`  contains all variable names needed to specify model `r mod_lbl` considered in this document
...
* Table with fold frequencies is provided.
...
length(foldid)
tibble(tx=foldid) %>% group_by(tx) %>% count()
...
* Note: Postfix *a0* in object names indicates that $\alpha=0$
...
```{r loss-function-plot}
theme_set(theme_minimal())

ggplot(cvfit_a0_mytidy, aes(x = log(lambda))) +
  geom_line(aes(y = estimate)) +
  geom_line(aes(y = conf.low),  color = "red", linetype="twodash") +
  geom_line(aes(y = conf.high), color = "red", linetype="twodash") +
  geom_vline(xintercept = log(lmbda_min_a0), color ="blue") +
  geom_vline(xintercept = log(lmbda_1se_a0), color ="blue", linetype="twodash")
