...
* Create `train_data15` data that is censored at 15 (prediction horizon)
...
save(vars,  sel3long, cva_modlist, glmnet_list, cvglmnet_list, file = fpath1) # train_data15 not saved
...
mod_all <- map(alphv, function(a){
  ...
  set_engine("glmnet") %>% 
  fit(Surv(time, status) ~ ., data = train_data15)
})
length(mod_all)
tmp <- mod_all[[2]]
class(tmp)
