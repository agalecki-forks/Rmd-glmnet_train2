...
* Create data set that is censored at 15 (time horizon)

```{r survSplit}
temp <- survSplit(Surv(time,  status) ~ ., data = train_data, cut = 15,
                  episode = "epsd")
dim(train_data)
train_data15 <- subset(temp, epsd == 1)  # only the first 15 years
dim(train_data15)
train_data    <- train_data15
```
...
dtx <- train_data %>% select(all_of(xvars)) 
x <- model.matrix(~0 +., data=dtx)
dim(x)
colnames(x)
# Create Surv object
y <- data.matrix(train_data[,c("time", "status")])
...
set.seed(1234)
nfolds <-10
foldid <- sample(1:nfolds, size = nrow(train_data), replace=TRUE)
length(foldid)
```
...
test_rows <- 1:100 # nrow(train_data) # Select rows for testing. Possibly all rows in train_data.
#x_test <- x[test_rows,]
#y_test <- y[test_rows,]
...
```{r save-train-objects, include =FALSE}
fpath1 <- paste0("./save/5.Cox_tidy_valid", mod_lbl, ".Rdata")
save(vars, train_data, sel3long, cva_modlist, file = fpath1)
#- fpath2 <- paste0("./save/5.Cox_tidy2_", mod_lbl, ".Rdata")
# save(mod_selected, cva_pfit, mod_all, sel3long, file = fpath2)
```

`test_data` contains all variables listed in `vars` vector (without missing values)
...
mod_all <- map(alphv, function(a){
  ...
  set_engine("glmnet") %>% 
  fit(Surv(time, status) ~ ., data = train_data)
})
length(mod_all)
tmp <- mod_all[[2]]
class(tmp)
