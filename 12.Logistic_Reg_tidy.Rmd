...
idvars <- "INDEX"
protein_nms <- c("KIM1.npx",       "SYND1.npx",   "IL.1RT1.npx", "WFDC2.npx", "CD27.npx", 
...
vars_othr <- c(idvars, tvars, varx1, varx2)


protein_df <- data_orig %>% select(all_of(protein_nms))

colnames(protein_df)

data_othr <- data_orig  %>% 
    select(all_of(vars_othr))
colnames(data_othr)


data_othr <- data_othr  %>%   
    mutate(status = CASE_CONTROL, time = FU_TIME)  %>% # rename variables
    mutate(log10_B_HBA1C_PRC = log10(B_HBA1C_PRC),
           log10_DU_ACR=log10(DU_ACR))   %>%
    select(-c(B_HBA1C_PRC, DU_ACR, CASE_CONTROL, FU_TIME))   # drop selected variables
data_all <- bind_cols(data_othr, protein_df)
...
varx <- c(protein_nms, tvars)

data <- data_all  %>% select(all_of(varx)) 

#drop any records with NA
data_drop_na <- data[, protein_nms] %>% drop_na()    

#Total sample size
nrow(data_drop_na)

#includes all proteins after excluding missing data
x <- data.matrix(data_drop_na[,protein_nms])
dim(x)
colnames(x)
