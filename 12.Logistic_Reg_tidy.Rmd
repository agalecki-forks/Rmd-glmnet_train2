...
data_all <- data_orig  %>%  
  ...
  mutate(status = factor(status, labels =c("0x","1x"))  
```
...
varnms1  <- c("INDEX", protein_nms, "status")
...
```{r initial-split}
set.seed(123)
splits     <- initial_split(data1, strata = status, prop=0.9)
...
```{r validation-split}
set.seed(234)
val_set <- validation_split(data1_train, strata = status, prop = 0.80)
val_set
...
*  glmnet model specification 

```{r glmnet-mod-spec}
glmnet_modspec <- 
   logistic_reg(penalty = tune(), mixture = 0) %>%  # Logistic regression
   set_engine("glmnet")
...
```{r M1-recipe}
glmnet_recipe <-
  recipe(status ~ ., data = data1_train) %>%
...
glmnet_tune <- 
  tune_grid(glmnet_wf, resamples = cv_set, grid = glmnet_grid) 
