...
dim(data_drop_na2)
test_rows <- 1:35    # Select rows for testing. Possibly all rows in data_drop_na2.
...
# Fit optimal model M2 using tidymodels
...
mod2all <-   proportional_hazards(penalty = tune(), mixture = a2_opt) %>% 
  set_engine("glmnet") %>% 
...
# Different survival probabilities for different subjects
predict(mod2, data2_test, type = "survival", time = c(5,10)) %>% 
