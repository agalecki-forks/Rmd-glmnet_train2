...
xvars0 <- c(surv_vars, clin_vars)
 
data0 <- scrambled  %>% select(all_of(xvars0)) 
...
data1 <- scrambled  %>% select(all_of(xvars1)) 
...
xvars2 <- c(surv_vars,  prot_npx, clin_vars)
...
pred_M2 <-predict(mod2_opt, data2_test, type = "time") %>% 
 cbind(data2_test) %>%
