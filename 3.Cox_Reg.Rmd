...
`status` indicates that an etime-to-follow-up is _censored_. 
...
# Time-dependent ROC for Survival Prediction Models
* source: (https://datascienceplus.com/time-dependent-roc-for-survival-prediction-models-in-r)

```{r surv-roc}
## Fit a Cox model
coxph2 <- coxph(formula = Surv(time, status) ~ pspline(AGE_TL, df = 4) + factor(SEX) + pspline(log10_B_HBA1C_PRC, df = 4)+ pspline(log10_DU_ACR, df = 4), data    = data_drop_na2)
                
## Obtain the linear predictor
data_drop_na2$lp <- predict(coxph2, type = "lp")
# data_drop_na2
