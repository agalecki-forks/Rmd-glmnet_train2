...
`status` (0/1) variable, originally named `CASE_CONTROL`, is coded 0 for Controls, and 1 for Cases.

`status` indicates that follow-up time is _censored_. 
...
Data are prepared for "unadjusted" cox regression (Model M1)
...
Cox regression coefficients for Model M1 using the best combination of (hyper)parameters
...
* Step 1: Fit the glmnet model to original data (stored in x2 and y2 matrices) using optimal alpha.
* Note: Object `pfit2_a` contains models for all lambdas. 

```{r pfit2_a-object}
a <- pfit2_params[["alpha"]]
lmbda_min <- pfit2_params[["lambdaMin"]]
pfit2_a <- glmnet(x2, y2Surv, family = "cox", alpha=a)
```
...
```{r calc-pred2-vals}
pred_lp <- predict(pfit2_a, newx = x2_test, type = "link", s = lmbda_min)

```

## Predictive performance of M2
