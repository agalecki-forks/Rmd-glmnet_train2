...
predM2_survprob <- 
   predict(mod2_opt, data2_test, type = "survival", time = c(5,10)) %>%
...
* Bind columns with predicted values  for M2

```{r cbind-predM2}
M2_pred <- 
   cbind(data2_test, predM2_time, predM2_lp ) %>% 
   select(time, status, predM2_time, predM2_lp) %>%
   as_tibble()
M2_pred

summ_M2 <- M2_pred %>%
                summarize(CindexM2 = Cindex(predM2_lp, Surv(time, status)))
summ_M2$CindexM2  

```

* Linear predictor is used as a marker for ROC curve

```{r ROC-surv-plot-m2tidy}
tx <- 2.5* c(2,3,4,5,6)    # ROC time points 
