...
* M1: Contains 21 proteins as candidate covariates _and_ interaction terms 
* M2 :Contains 21 proteins,Baseline HbA1c, log10(ACR), BL_eGFR, SEX, and AGE_TL(Baseline Age) as candidate covariates (and intercation terms)
...
f <- as.formula(Surv(time, status) ~.*.)
x <- model.matrix(f, data)[, -1]  # Drop intercept
dim(x)
...
```{r save-cva_pfit1}
save(cva_pfit1, file = "./save/4Cox-Reg1-save1.Rdata")
```
...
```{r save-cva_pfit2_M2}
save(cva_pfit2, file = "./save/4Cox-Reg1-save2.Rdata")
```
